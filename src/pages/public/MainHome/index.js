...
const Home = () => {
    ...
    const { homePageWearableContent, homePageArtContent } = useSelector(item => item?.profileReducer);
    ...
    useEffect(() => {
        if (homePageWearableContent?.results) {
            formik.setFieldValue("wearablePaginationTotalCount", homePageWearableContent?.count);
    ...
    useEffect(() => {
        if (homePageArtContent?.results) {
            formik.setFieldValue("artPaginationTotalCount", homePageArtContent?.count);
            formik.setFieldValue("artData", homePageArtContent?.results);
        }
    }, [homePageArtContent]);


    const onScrollSection = (currentPaginationCount, totalPaginationCount, fieldName) => {
        if (
            Math.round(totalPaginationCount / 10 * 100) / 100 >= currentPaginationCount
        ) {
            ...
            <div className={styles.main}>
                ...
                >
                    <section
                        ...
                        ref={scrollRef}
                    >
                        {formik.values.artData.length ? formik.values.artData.map((el) => (
                            <div key={el.name + el.id + Math.random()}>
                                <HomeScreenItem
                                    onPressItem={() => onPressItem(el.id, el.slug)}
                                    imageUrl={
                                        el?.productphoto_set[0]?.photo?.image
                                    }
                ...
                <ScrollContainer
                    ...
                    <section
                        ...
                        ref={scrollRef}
                    >
                        {formik.values.wearableData.length ? formik.values.wearableData.map((el) => (
                            <div key={el.name + el.id + Math.random()}>
                                <HomeScreenItem
                                    onPressItem={() => onPressItem(el.id, el.slug)}
                                    imageUrl={
                ...
                <ScrollContainer className={styles.container}>
                    ...
                    >
                        {architectureData.map((el) => (
                            <div key={el.id}>
                                <HomeScreenItem imageUrl={el.image} onPressItem={onToggleModal} title={el.title} />
                            </div>
                        ))}
                    </section>
                </ScrollContainer>
